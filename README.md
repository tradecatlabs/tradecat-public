...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/v0.1.3/install.sh | sh
tradecat
```
...
```powershell
irm https://raw.githubusercontent.com/tukuaiai/tradecat/v0.1.3/install.ps1 | iex
tradecat
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/v0.1.3/install.sh | TRADECAT_INSTALL_SKIP_SYNC=1 TRADECAT_INSTALL_SKIP_PATH_WRITE=1 sh
```

Windows PowerShell：

```powershell
$env:TRADECAT_INSTALL_SKIP_SYNC = "1"; $env:TRADECAT_INSTALL_SKIP_PATH_WRITE = "1"; irm https://raw.githubusercontent.com/tukuaiai/tradecat/v0.1.3/install.ps1 | iex
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/install.sh | TRADECAT_INSTALL_BRANCH=develop sh
```

Windows PowerShell：

```powershell
$env:TRADECAT_INSTALL_BRANCH = "develop"; irm https://raw.githubusercontent.com/tukuaiai/tradecat/develop/install.ps1 | iex
```
...
`skills/tradecat-public/references/dataset-consumption-contract.md`。
...
给人和 Hermes 共用的安装/运行/安全指南见 `skills/tradecat-public/references/hermes-agent-guide.md`；机器主契约是 `skills/tradecat-public/agents/manifest.json`。
...
JSONL cycle archive 和 paper ledger，生成可复现纸面回放/回测摘要。持续 paper/watch 服务默认把运行态写入 `.runtime/auto-paper/`：`service_state.json`、`paper_ledger.json`、`cycles.jsonl`、`paper_audit.sqlite3`、PID、heartbeat 和 log 都是本地运行态，已加入 `.gitignore`，不得提交；`audit-journal`、`health-report`、`daily-report` 和 `alert-payload` 只读取这些本地文件生成审计与告警 payload，不触发真实交易。
...
如果 operator 明确授权 AI/Agent 纸面自治，可把 `tradecat_auto.paper_autonomy_profile.v1` 放在 `.runtime/auto-paper/`，并设置 `TRADECAT_AUTO_PAPER_AUTONOMY_PROFILE_PATH` 后重启服务。该 profile 只能补齐 paper/watch sizing、exit plan、同币种 paper 多仓授权，以及显式允许时的 paper-only `direction_conflict` 覆盖；它不是 Binance 订单，不允许 key、签名、账户读取或真实下单字段。
...
| 变量 | 默认值 | 说明 |
|:---|:---|:---|
| `TRADECAT_SETTINGS_PATH` | TradeCat 项目根 `.tradecat/settings.json` | 用户侧配置文件路径 |
| `TRADECAT_CACHE_DIR` | TradeCat 项目根 `.tradecat/cache` | 本地快照缓存目录 |
| `TRADECAT_PUBLIC_ROOT` | 自动识别当前仓库根 | `tradecat auto` 读取本仓 `scripts/request.py` 时的仓库根覆盖 |
...
| `TRADECAT_REQUEST_REGISTRY_URL` | 仓库内 `src/tradecat_terminal/dataset_registry.json`；零安装脚本找不到本地 registry 时 fallback GitHub develop | 一次性请求脚本读取的 dataset registry JSON |
...
不使用根脚本时，也可以在仓库根目录手动创建 `.venv` 并执行
`pip install -c constraints.txt -e ".[dev]"`。

CI 会执行：

- 根/Skill 边界门禁：根目录保持正常 Python 项目；Skill 包只在 `skills/tradecat-public/`。

...
- **影响范围**：
  ...
  - 数据源：TimescaleDB (端口 5433)、PostgreSQL 指标/状态库（`tg_cards.*` / `signal_state.*` / `sheets_state.*`）
...
### 设计选择与实现要点
1. **技术栈**：FastAPI + Uvicorn + Pydantic + psycopg（PostgreSQL）
...
| /futures/supported-coins | PG（tg_cards / 全局配置） | tg_cards."基础数据同步器.py" 或 assets/common/symbols.py |
| /indicator/* | PG（tg_cards） | tg_cards.*（指标表） |
| /signal/cooldown | PG（signal_state） | signal_state.cooldown |

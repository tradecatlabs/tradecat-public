...
def test_readonly_cli_json_contracts(tmp_path, capsys, monkeypatch):
    ...
    assert cli.main(["--cache-dir", str(cache_dir), "path", "signal_flow", "--json"]) == 0
...
def test_mutating_cli_json_contracts(tmp_path, capsys, monkeypatch):
    ...
    assert cli.main(["--cache-dir", str(cache_dir), "probe", "signal_flow", "--no-write", "--json"]) == 0
...
def test_export_and_bundle_json_contracts(tmp_path, capsys):
    ...
    assert cli.main(["--cache-dir", str(cache_dir), "export", "signal_flow", "--format", "json", "--limit", "1"]) == 0
...
def test_json_errors_have_stable_error_object(tmp_path, capsys):
    ...
    assert payload["error"] == {
        ...
        "kind": "validation",
        "message": "未知 dataset_key: missing; 可用值: anomaly_panel, event_stream, market_snapshot, market_stats, signal_flow",

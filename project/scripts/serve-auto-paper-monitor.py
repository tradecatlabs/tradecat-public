...
DEFAULT_RUNTIME_DIR = PROJECT_DIR / ".runtime" / "auto-paper"
...
def build_decision_text(latest_cycle: dict[str, Any]) -> dict[str, Any]:
    ...
    if not pipeline:
        return {
            ...
            "sections": [],
            "provenance": {
                "source": "project/.runtime/auto-paper/cycles.jsonl",
...
def build_snapshot(runtime_dir: Path) -> dict[str, Any]:
    ...
    status = run_json(["bash", "project/scripts/start-auto-paper.sh", "status", "--json"], runtime_dir=runtime_dir)
    health = run_json(["bash", "project/scripts/start-auto-paper.sh", "health", "--json"], runtime_dir=runtime_dir)
    ops = run_json(["bash", "project/scripts/start-auto-paper.sh", "ops-check", "--json"], runtime_dir=runtime_dir)
    paper = run_json(
        [
            "bash",

# Repo Evidence
- git status 显示 develop ahead 13，且存在未提交的 project/src/tradecat_auto 与 project/tests 补丁，以及未跟踪 agent_trade_thesis.py。
- AGENTS.md 明确根目录是 Skill 外壳，project/ 是唯一 Python 项目根，根目录禁止 assets/src/tests/pyproject/Makefile。
- agents/manifest.json 已声明 agent_market_context、agent_trade_thesis、automation service、runtime paths 和错误码。
- project/src/tradecat_auto 已有 context-audit/run-context、paper ledger、audit journal、health report 和 start-auto-paper。
...
- 任务文档落在 project/tasks/，避免违反根目录禁止 assets/ 的治理边界。
- 执行实现时必须保持最小改动，不混入无关重构。
- 任何后台 loop 启动必须由用户单独授权；本任务树本身只定义和规划。

# Change Boundary
- 允许新增 project/tasks/0001-autonomous-agent-trader-loop/ 任务文档和机器任务包。
- 允许最小更新 project/AGENTS.md 记录 project/tasks/ 目录职责。
...
- project/tasks/ 是本仓适配 auto-tasks 的项目内任务目录。
...
- 依赖节点 ID: 无
- 输入: 无
- 输出: project/contracts/tradecat-auto-agent-research-cycle.schema.json
- 风险: 无
- 备注: 无
...
- 依赖节点 ID: TP-02.01
- 输入: 无
- 输出: project/tests/fixtures/agent_research_cycle/*.json
- 风险: 无
- 备注: 无

...
2. 进入仓库内 `project/` 项目目录并创建项目内 `.venv`。
...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/v0.1.3/project/install.sh | TRADECAT_INSTALL_SKIP_SYNC=1 TRADECAT_INSTALL_SKIP_PATH_WRITE=1 sh
```

Windows PowerShell：

```powershell
$env:TRADECAT_INSTALL_SKIP_SYNC = "1"; $env:TRADECAT_INSTALL_SKIP_PATH_WRITE = "1"; irm https://raw.githubusercontent.com/tukuaiai/tradecat/v0.1.3/project/install.ps1 | iex
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/install.sh | TRADECAT_INSTALL_BRANCH=develop sh
```

Windows PowerShell：

```powershell
$env:TRADECAT_INSTALL_BRANCH = "develop"; irm https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/install.ps1 | iex
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/uninstall.sh | sh
```

Windows PowerShell：

```powershell
irm https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/uninstall.ps1 | iex
```
...
| `TRADECAT_PROJECT_SUBDIR` | 覆盖仓库内项目子目录，默认 `project` |
...
```bash
python3 <(curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py) event_stream
```

`request.py` 会读取同仓库的 `project/src/tradecat_terminal/dataset_registry.json`，与安装版使用同一个 dataset 契约；可用 `TRADECAT_REQUEST_REGISTRY_URL` 或 `--registry-url` 覆盖。
...
```bash
# 列出可用 dataset
python3 <(curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py) --datasets

# 查看事件流前 20 行
python3 <(curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py) event_stream --limit 20

# JSONL 输出，方便脚本或 Agent 消费
python3 <(curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py) market_snapshot --format jsonl --limit 10

# 只看元信息或表头
python3 <(curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py) market_stats --meta
python3 <(curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py) anomaly_panel --headers
```

不支持 process substitution 的 shell 可使用：

```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/project/scripts/request.py -o /tmp/tradecat-request.py
...
2. 进入仓库内 project 目录后创建 Python 3.12 虚拟环境
...
| 变量 | 默认值 | 说明 |
|:---|:---|:---|
| `TRADECAT_SETTINGS_PATH` | TradeCat 项目根 `project/.tradecat/settings.json` | 用户侧配置文件路径 |
| `TRADECAT_CACHE_DIR` | TradeCat 项目根 `project/.tradecat/cache` | 本地快照缓存目录 |
| `TRADECAT_PUBLIC_ROOT` | 自动识别当前仓库根 | `tradecat auto` 读取本仓 `project/scripts/request.py` 时的仓库根覆盖 |
...
不使用根脚本时，也可以在 `project/` 内手动创建 `.venv` 并执行
`pip install -c constraints.txt -e ".[dev]"`。

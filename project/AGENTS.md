...
本文件作用域：`tradecat-public/project/**`。

## 使命

`tradecat-public` 首先是给用户本机 Hermes 安装/加载的 Skill 包；真实 TradeCat 用户侧消费端项目位于 `project/`。该项目提供只读公开在线表格、本地快照缓存、CLI/TUI、Agent 观察/事实包，以及 Agent-supplied market context 的安全审计/纸面运行契约。
...
-> 节点2：`tradecat_source.py` 调用本仓 `project/scripts/request.py` 读取公开 sheet 行，按 `source_time_bj + content` 生成稳定 event_id
...
```text
Input(输入)：`python3 <(curl .../project/scripts/request.py) <dataset_key>`、公开 dataset registry JSON、公开 Google Sheets CSV
-> 节点1：`project/scripts/request.py` 读取 `dataset_registry.json` URL 并解析 format/limit/meta/headers 参数
-> 节点2：`project/scripts/request.py` 根据共享 registry 生成 Google Sheets CSV export URL，不创建本地缓存，不依赖 Python 第三方库
-> 节点3：脚本解析顶部信息、表头和业务行
...
```text
Input(输入)：`curl .../project/install.sh | sh` 或 `irm .../project/install.ps1 | iex`
...
-> 节点4：安装脚本定位仓库内 `project/` 项目目录，创建 `.venv` 并执行 editable install
...
```text
Input(输入)：`tradecat-uninstall`、`curl .../project/uninstall.sh | sh` 或 `irm .../project/uninstall.ps1 | iex`
-> 节点1：卸载脚本解析安装目录、bin 目录、运行态目录和 `TRADECAT_KEEP_CACHE`
-> 节点2：如果 `TRADECAT_KEEP_CACHE=1`，卸载脚本优先把 `project/.tradecat/cache` 移动到用户目录备份；旧布局缓存作为兼容回退
...
- `config.py`：本地缓存目录与环境变量解析；默认缓存根为项目根 `project/.tradecat/cache`。
...
- `scripts/request.py`：零安装一次性公开数据请求脚本；公开 curl 路径为 `project/scripts/request.py`；读取共享 registry，只能用标准库，JSON 模式必须输出 `tradecat.request_result.v1`。
...
- `sync/probe/watch` 只能写 `TRADECAT_CACHE_DIR` 下的缓存文件；未设置时固定写项目根 `project/.tradecat/cache`。
...
- 新增配置、缓存路径、结构化 JSON 字段或公开请求参数必须同步更新 `project/README.md`。
- `project/scripts/request.py` 是公开 curl 入口，必须保持无第三方依赖、无本地写入、无私密源，并且 dataset/workbook/gid 必须来自共享 `dataset_registry.json`。

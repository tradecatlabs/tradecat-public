...
- 入口脚本：`libs/database/db/stacks/lf.sql`
...
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d market_data \
  -f libs/database/db/stacks/lf.sql
```

### HF（高频/原子事实库）

- 目标对象：`core.*` + `storage.*` + `crypto.raw_*`（逐笔/订单簿/治理旁路）
- 入口脚本：`libs/database/db/stacks/hf.sql`
...
PGPASSWORD=postgres psql -h localhost -p 15432 -U postgres -d market_data \
  -f libs/database/db/stacks/hf.sql
```
...
- `libs/database/db/legacy_projects_tradecat/`：从旧项目路径拷贝的 LF DDL 快照（不要作为当前真相源）。

...
[ ] P0: 迁移脚本准备（rename-swap + integer hypertable(us)） | Verify: `ls -la assets/database/db/schema | rg -n "015_.*spot.*trades"` | Gate: 脚本存在且影响范围可控  
[ ] P0: 执行迁移脚本 | Verify: `psql "$DATABASE_URL" -f assets/database/db/schema/015_crypto_spot_trades_fact_table.sql` | Gate: exit code=0  

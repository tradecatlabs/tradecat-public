...
- 单次 dry-run probe 的 `tradecat.probe_result.v1` 是正式 Agent 契约；watcher 生命周期控制面（`start.sh status/start/stop --json` 与 `watchdog.sh --json`）的 `tradecat.watch_status.v1` 是正式 Agent 契约；`tradecat.watch_cycle.v1` 仍是内部长运行契约，未进入 manifest 前不得写成 Agent 正式承诺面。

...
if not ok:
    payload["error"] = {
        ...
        "hint": "Run scripts/project/scripts/start-auto-paper.sh start --json, then inspect paper-report and .runtime/auto-paper/cycles.jsonl.",
        "retryable": False,
    }
...
start() {
  ...
  (
    cd "$APP_DIR"
    nohup bash "$APP_DIR/scripts/start-auto-paper.sh" _run >>"$LOG_FILE" 2>&1 &
    echo "$!" >"$PID_FILE"
  )
  local pid
  pid="$(read_pid)"

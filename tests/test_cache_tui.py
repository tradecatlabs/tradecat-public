...
def test_root_ci_uses_pinned_secret_scan_and_bootstrap_script():
    ...
    assert "https://raw.githubusercontent.com/tukuaiai/tradecat/${TRADECAT_PUBLIC_INSTALL_REF}/install.sh" in ci_yml
    ...
    assert "validate_dataset_consumption_contract.py" in (REPO_ROOT / "scripts" / "verify-project.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert "bootstrapping $ROOT_DIR/.venv" in (SKILL_ROOT / "scripts" / "verify.sh").read_text(encoding="utf-8")
...
def test_release_version_metadata_matches_stable_ref():
    ...
    release_notes = (SKILL_PACKAGE_ROOT / "references" / "release.md").read_text(encoding="utf-8")

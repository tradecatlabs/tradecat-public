...
def test_agent_manifest_is_canonical_machine_contract():
    manifest_path = SKILL_PACKAGE_ROOT / "agents" / "manifest.json"
...
def test_skill_package_governance_is_machine_readable_and_boundary_safe():
    ...
    assert governance["skill_root"] == "skills/tradecat-public"
    assert governance["implementation_project_root"] == "."
    assert governance["no_second_truth"] is True
    assert {"SKILL.md", "agents", "references", "project/src"} <= set(governance["forbidden_root_paths"])
...
def test_skill_package_boundary_has_no_forbidden_tracked_or_root_files():
    ...
    assert not (REPO_ROOT / "scripts" / "project").exists()
...
def test_manifest_important_static_paths_exist_and_runtime_paths_are_local_only():
    ...
    assert payload["important_paths"]["skill_package_governance_reference"] == (
        "skills/tradecat-public/references/skill-package-governance.md"
    )
...
def test_manifest_advertises_tradecat_auto_lifecycle_entrypoints():
    ...
    assert "bash scripts/start-auto-paper.sh status --json" in commands
    assert "bash scripts/start-auto-paper.sh ops-check --json" in commands
    ...
    assert payload["important_paths"]["automation_source"] == "src/tradecat_auto"
    assert payload["important_paths"]["automation_audit_journal"] == ".runtime/auto-paper/paper_audit.sqlite3"
...
def test_agent_profiles_point_to_manifest_instead_of_second_truth():
    openai = (SKILL_PACKAGE_ROOT / "agents" / "openai.yaml").read_text(encoding="utf-8")
    hermes = (SKILL_PACKAGE_ROOT / "agents" / "hermes.yaml").read_text(encoding="utf-8")

    assert "manifest: skills/tradecat-public/agents/manifest.json" in openai
    assert "manifest: skills/tradecat-public/agents/manifest.json" in hermes
    assert "skill_package_governance: skills/tradecat-public/references/skill-package-governance.md" in openai
    assert "skill_package_governance: skills/tradecat-public/references/skill-package-governance.md" in hermes
    assert "agent_contract: skills/tradecat-public/references/agent-contract.md" in openai
    assert "agent_contract: skills/tradecat-public/references/agent-contract.md" in hermes
    assert "canonical_source: skills/tradecat-public/agents/manifest.json#agent_role_profiles" in openai
    assert "canonical_source: skills/tradecat-public/agents/manifest.json#agent_role_profiles" in hermes
...
def test_agent_contract_reference_is_indexed():
    index = (SKILL_PACKAGE_ROOT / "references" / "index.md").read_text(encoding="utf-8")
    contract = (SKILL_PACKAGE_ROOT / "references" / "agent-contract.md").read_text(encoding="utf-8")
    guide = (SKILL_PACKAGE_ROOT / "references" / "hermes-agent-guide.md").read_text(encoding="utf-8")
    manifest = json.loads((SKILL_PACKAGE_ROOT / "agents" / "manifest.json").read_text(encoding="utf-8"))
    ...
    assert "skills/tradecat-public/references/skill-package-governance.md" in manifest["human_docs"]
    assert "skills/tradecat-public/references/skill-package-governance.md" in manifest["agent_docs"]
    assert "skills/tradecat-public/references/hermes-agent-guide.md" in manifest["human_docs"]
    assert "skills/tradecat-public/references/hermes-agent-guide.md" in manifest["agent_docs"]
    assert "skills/tradecat-public/references/agent-soft-decision-layer.md" in manifest["agent_docs"]
    assert "skills/tradecat-public/references/private-executor-boundary.md" in manifest["agent_docs"]
    assert manifest["important_paths"]["skill_package_governance_reference"] == (
        "skills/tradecat-public/references/skill-package-governance.md"
    )
    assert manifest["important_paths"]["hermes_agent_guide"] == "skills/tradecat-public/references/hermes-agent-guide.md"
    assert manifest["important_paths"]["private_executor_boundary_reference"] == (
        "skills/tradecat-public/references/private-executor-boundary.md"
    )
    assert manifest["important_paths"]["agent_soft_layer_resources"] == "resources/agent_soft_layer"
    assert manifest["important_paths"]["agent_soft_layer_trader_profile"] == "resources/agent_soft_layer/profiles/discretionary-futures-trader.zh.md"
...
def _load_cli_schemas() -> dict[str, str]:
    module_path = REPO_ROOT / "src" / "tradecat_terminal" / "contracts.py"

...
`project/.runtime/auto-paper/` 本地纸面运行态；不读取 Binance key、不签名、不调用账户或订单接口、不真实下单。
...
```bash
TRADECAT_AUTO_PAPER_AUTONOMY_PROFILE_PATH=project/.runtime/auto-paper/paper_autonomy_profile.json \
  bash project/scripts/start-auto-paper.sh restart --json
```

使用 user systemd timer 作为长期 keepalive：

```bash
bash project/scripts/start-auto-paper.sh systemd-install --json
...
```bash
bash project/scripts/start-auto-paper.sh health --json
bash project/scripts/start-auto-paper.sh daily --json
bash project/scripts/start-auto-paper.sh alert --json
```

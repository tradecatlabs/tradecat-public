...
### 8.2 多周期“字段为行 / 周期为列”展示规则（新增：字段纵向、周期横向）
当你希望 **周期 7 个固定横向（x 轴）**、其它字段按行展示（y 轴）以提升冻结与阅读一致性时：
- 表头：`币种 | 字段 | 1m | 5m | 15m | 1h | 4h | 1d | 1w`
- 行：每个币种按字段拆成多行；每行一个字段，右侧 7 列分别为 7 个周期的值
- 冻结建议：冻结前 2 列（`币种`、`字段`）
- 配色建议：对 7 个周期列做灰白交替，便于横向扫读

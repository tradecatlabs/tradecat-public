...
- `skill-package-governance.md`: Hermes Skill 包形态、仓库根 Python 项目职责、`skills/tradecat-public/` Skill 边界、Agent/交易员 role profile 和运行态隔离规则。
...
The Python project lives at the repository root. Load project source files only when the task requires code-level changes; load `skills/tradecat-public/` for Skill activation, Agent manifest, platform profile, and reference docs.

...
- Current baseline at the time of this historical plan: root was a Skill
  wrapper and the Python project lived under the legacy nested project
  directory; current layout has
  since moved the Python project to the repository root with the Skill package
  embedded under `skills/tradecat-public/`.
...
- Formal schema drafts live under `contracts/`.
...
- Maintain `DEBUG.md` while debugging `TP-01` and `TP-04`.

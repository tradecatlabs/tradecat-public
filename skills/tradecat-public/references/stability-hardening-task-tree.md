...
| --- | --- | --- | --- |
| P1 | Remote fetch has no retry, jitter, or error classification. | `src/tradecat_terminal/sheets.py:9` | HTTP transport with bounded retry/backoff/jitter and typed errors. |
| P1 | Local cache/settings writes lack process-level locking. | `src/tradecat_terminal/cache.py:176` | Cross-platform file locks and transactional local state updates. |
| P1 | Settings writes are not atomic and corrupt JSON is silently ignored. | `src/tradecat_terminal/settings.py:32`, `settings.py:43` | Atomic write, `.bak`, corrupt-file diagnostics, and repair path. |
| P2 | Dependencies are range-based, not locked. | `pyproject.toml:16` | `uv.lock` or constraints with drift checks and release evidence. |
| P2 | Installer can bootstrap uv from a remote script. | `install.sh:52` | Fixed version/checksum or explicit opt-in. |
...
| P2 | Doctor lacks a support bundle. | `src/tradecat_terminal/lifecycle.py:15` | `doctor --verbose --bundle --repair` with safe diagnostics. |
| P2 | Cache schema has versions but no migration framework. | `src/tradecat_terminal/cache.py:128` | Explicit migrations, backup, rollback, fixtures. |
| P3 | Permanent snapshots lack disk-waterline warnings. | `src/tradecat_terminal/cache.py:224` | Doctor cache-size warnings and prune guidance. |
...
```bash
curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/<next-tag>/install.sh | sh
...
- For `TP-01` and `TP-02`, maintain `DEBUG.md` while debugging failures.

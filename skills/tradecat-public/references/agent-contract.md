...
This repository is the TradeCat public Python project with an embedded
multi-Agent Skill package in `skills/tradecat-public/`. The canonical
machine-readable contract is `skills/tradecat-public/agents/manifest.json`;
platform files such as `skills/tradecat-public/agents/openai.yaml` and
`skills/tradecat-public/agents/hermes.yaml` are thin adapters and must not
become second sources of truth.

Human + Hermes operating guide: `skills/tradecat-public/references/hermes-agent-guide.md`. Use it when the task is about local Hermes skill installation, development-vs-production boundaries, or Agent-supplied Binance market context.

## Hermes Skill Consumption

When this Skill is installed under `~/.hermes/skills/tradecat-public`, Hermes should load `SKILL.md` first and then treat this file plus `agents/manifest.json` as the Skill-local view of the operating contract. Commands in the manifest are repository-root commands; in this checkout, the repository root is the Python project root, and `skills/tradecat-public/scripts/` only contains thin wrappers back to root scripts.
...
`src/tradecat_terminal/dataset_consumption_contract.json` or
...
| `network_readonly` | Reads public network data without writing cache. | `scripts/request.py signal_flow --format json` |
| `local_cache_write` | Writes only local TradeCat cache, settings, or diagnostics. | `init`, `sync`, `doctor --repair`, `prune --apply` |
| `background_long_running` | Starts or supervises a local watcher. | `scripts/start.sh start`, `watch` |
...
Formal schema files live in `contracts/`. The command-level
...
`tests/test_payload_schema_validation.py`. It validates live
CLI/request JSON output and golden samples from
`tests/fixtures/json_contract/` against the formal schemas.
...
`tests/test_agent_contract.py`, and
`tests/test_payload_schema_validation.py` in the same change.
...
- Research-cycle schema file: `contracts/tradecat-auto-agent-research-cycle.schema.json`.
- Payload schema: `tradecat_auto.agent_market_context.v1`.
- Schema file: `contracts/tradecat-auto-agent-market-context.schema.json`.
- Paper position-management thesis schema: `tradecat_auto.position_management_thesis.v1`.
- Position-management schema file: `contracts/tradecat-auto-position-management-thesis.schema.json`.
...
- Portfolio risk policy schema: `tradecat_auto.portfolio_risk_policy.v1`; schema file: `contracts/tradecat-auto-portfolio-risk-policy.schema.json`.
- Future private executor handoff draft schema: `tradecat_auto.audited_intent_handoff.v1`; schema file: `contracts/tradecat-auto-audited-intent-handoff.schema.json`.
- Source/provenance manifest: `resources/agent_market_context/binance/provenance.manifest.json`.
...
`.runtime/auto-paper/`: `service_state.json`, `paper_ledger.json`,
...
Use `bash scripts/start-auto-paper.sh status --json` to inspect
...
1. Production path: `tradecat_terminal.sheets.fetch_csv_body`, used by installed
   CLI sync/probe and `scripts/validate_data_contract.py`.
   It uses `urllib3` retry/backoff/jitter and typed `RemoteCsvError`.
2. Zero-install fallback: `scripts/request.py`, kept standard
   ...
   `src/tradecat_terminal/dataset_registry.json`; raw zero-install
...
`scripts/start.sh --json` is the machine-readable watcher

...
The repository root is the bundled Python project. Installer logic lives in
`install.sh` and `install.ps1`; the embedded Skill package lives under
...
- Locate the project inside the repository by `TRADECAT_PROJECT_SUBDIR`, defaulting to `.`.
...
- Install the bundled project from the repository root; when `constraints.txt`
...
- `TRADECAT_KEEP_CACHE=1` preserves `.tradecat/cache` before deleting the install directory.

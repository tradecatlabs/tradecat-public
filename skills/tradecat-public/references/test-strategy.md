...
TradeCat public is a Python CLI/TUI project with an embedded Hermes/Codex Skill
package. The repository root is the runnable user project; the Skill shell lives
under `skills/tradecat-public/`.
...
Agent -> skills/tradecat-public/agents/manifest.json -> status/datasets/path -> request fallback/sync
...
|---|---|---|---|
| Agent contract | `skills/tradecat-public/agents/`, `contracts.py`, `contracts/` | schema drift, ambiguous command risk | `test_agent_contract.py`, `agent-smoke.sh` |
...
| Type | Scope | Command |
|---|---|---|
| Unit | Pure helpers, parsers, state, schema utilities | `PYTHONPATH=src pytest -q tests/test_transport.py tests/test_state_migrations.py` |
| Contract | Manifest/schema/payload compatibility | `PYTHONPATH=src pytest -q tests/test_agent_contract.py tests/test_payload_schema_validation.py` |
| CLI | Exit codes, invalid input, JSON errors | `PYTHONPATH=src pytest -q tests/test_exit_codes.py tests/test_cli_boundaries.py` |
| Integration | Cache writes, TUI fallback, watcher scripts | `PYTHONPATH=src pytest -q tests/test_cache_tui.py` |

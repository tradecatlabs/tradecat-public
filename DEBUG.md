...
- `bash scripts/verify.sh` 已通过，覆盖 typed remote error、settings corrupt
...
- 用户在 WSL 内执行 `curl -fsSL https://raw.githubusercontent.com/tukuaiai/tradecat/develop/install.sh | sh`。
...
- 给 `src/tradecat_auto/*.py` 内公开 `tradecat_auto.*.v1` payload 补 `schema_version=1.0.0`。
